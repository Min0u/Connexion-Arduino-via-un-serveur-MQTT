...
function onMessageArrived(r_message){// quand un message arrive
	try {
		...
		if(dataJson.hasOwnProperty('ppmCO2')){ // si dans le json la clé 'E_Lux' existe...

			temps = Number(dataJson.temps);
			TVOC = dataJson.TVOC;
			CO2 = Number(dataJson.ppmCO2);
			gauge1.value = CO2;// mets à jour la jauge éclairement
			$('#ECL1').html(CO2);// rajoute la valeur de l'éclairement dans l'élément HTML d' ID ECL1 
			...
			if (AffichePoints) {// si on veut afficher les points sur la graphe
				...
				csvdata.push([currencyFormatFR(temps),currencyFormatFR(CO2)]);
